...
import { LayoutDashboard, ShoppingCart, CheckSquare, Settings, Shirt, Archive, Tags, Users, Menu, X, LogOut } from 'lucide-react';
...
export function AppLayout() {
  ...
  return (
    <div className="flex h-screen w-full bg-slate-50 text-slate-900 font-sans flex-col md:flex-row">
      {/* Mobile Header */}
      <header className="md:hidden flex items-center justify-between p-4 bg-white border-b border-slate-200 z-20 shrink-0">
      ...
      <aside className={cn(
        "fixed inset-y-0 right-0 z-30 w-64 border-l border-slate-200 bg-white flex flex-col transform transition-transform duration-300 ease-in-out md:relative md:translate-x-0",
        ...
        <div className="p-4 border-t border-slate-200 text-xs text-slate-500 shrink-0">
          <div className="flex justify-between items-center mb-2">
            ...
            <button onClick={logOut} className="p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-md transition-colors" title="تسجيل الخروج">
              <LogOut size={16} />
            </button>
          </div>
